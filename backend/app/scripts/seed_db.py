...
async def seed_db():
    async with async_session_maker() as session:
        ...
        # Create the first user first to act as the creator for global selections
        first_user = User(
        ...
        print(f"Created initial user: {first_user.email}")

        # --- Create Global Selects ---
        ...
        for name in tag_names:
            tag = Tag(title=name, color=random.choice(["red", "blue", "green", "yellow", "purple", "orange", "pink", "cyan"]), creator_id=first_user.id)
            session.add(tag)
            tags.append(tag)
        
        priorities = []
        for name in priority_names:
            priority = Priority(title=name, color=random.choice(["gray", "blue", "orange", "red"]), creator_id=first_user.id)
            session.add(priority)
            priorities.append(priority)
            
        statuses = []
        for name in status_names:
            status = Status(title=name, color=random.choice(["gray", "blue", "purple", "yellow", "green", "red", "black"]), creator_id=first_user.id)
            session.add(status)
            statuses.append(status)
            
        await session.commit()
        print("Created global tags, priorities, and statuses")

        # Create remaining 9 users
        users = [first_user]
        for i in range(1, 10):
            user = User(
        ...
        # Create Folders and Applications for all users
        for user in users:
            ...
            for j, folder_name in enumerate(folder_names):
                folder = Folder(title=folder_name, creator_id=user.id, position=j)
                session.add(folder)
                await session.commit()
...
"""
                ...
                await session.commit()
                print(f"Created folder {folder.title} with applications for {user.email}")
